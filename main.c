...
TODO	Support multibyte characters
	...
	Mostly in Gemini, when multibyte characters are used then
	all the line wrapping logic and underline drawing fails.
...
loadpage(char *link)
{
	...
	char *host, *path, *cache, *search, *pt, *cmd, *mime_str;
	...
	case GEMINI:
		...
		pt = 0;
		if ((why = gmi_onheader(res, &mime_str, &pt)))
			return why;

		if (mime_str)
			mime = mime_header(mime_str);
